...
一个帮助应届毕业生和初到深圳的年轻人关注过渡性住房的开源 Demo，主要适配微信“安居乐寓”小程序里的“深梦扬帆”房源。

找房时不必反复刷新小程序。程序在你的电脑本地定时检查房源，发现关注区域有新房或余量增加时发送邮件；如果收件邮箱已经开启微信提醒，就能直接在微信里看到消息。

本项目只做公开房源信息的监控和通知，不会自动登录、自动报名、自动预约或代抢。最终房源和活动规则请以官方小程序为准。

## 为什么做这个 Demo

应届毕业生刚来深圳时，往往同时面对找工作、搬家和租房。价格相对友好的过渡房源数量有限，手动反复查看既耗时间，也容易错过更新。
...
> 请查看我已经打开的微信“安居乐寓—深梦扬帆”房源页面，在当前文件夹创建一个只在本地运行的房源监控工具。让我选择需要关注的行政区、街道或社区，每 10 分钟检查一次，记录房源增加和减少；关注区域出现新房时，用我已登录的 Gmail 给我指定的、能在微信中提醒的邮箱发消息。程序要有“立即检查”“发送测试邮件”“开始监控”和“停止监控”按钮，只负责提醒，不自动报名。个人邮箱和登录信息不得上传到 GitHub。需要登录或授权时再让我操作，其他步骤请你完成，并在交付前实际运行测试。最后整理好 README，让我可以直接发布到 GitHub。
