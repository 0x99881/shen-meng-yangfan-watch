...
你不需要研究代码，也不需要自己再做一个项目。准备好小程序和邮箱，把下面的话交给本地 AI，直接让它完成检查、测试和后续监控。

### 准备

1. 在微信中打开并登录“安居乐寓”小程序，进入“深梦扬帆”房源页面。
2. 在电脑上登录准备用来发信的 Gmail。
3. 准备一个已经绑定微信提醒的 QQ 邮箱，用来接收房源消息。
4. 打开 Codex，推荐选择 **GPT-5.6 Sol**，推理强度选择 **极高**。
5. 把下面的提示词直接发给它。

### 可以直接复制的提示词

> “深梦扬帆”是帮助应届毕业生来深圳免费住宿 15 天的公益活动。请查看我已经打开的微信“安居乐寓—深梦扬帆”房源页面，直接帮我在本地监控房源。先让我选择关注的行政区和检查间隔，默认每 10 分钟检查一次，并记录房源增加和减少。关注区域出现新房时，用我已经登录的 Gmail 给我指定的、已经绑定微信提醒的 QQ 邮箱发送消息。请先主动发送一封测试邮件，确认邮件发送正常，再开始持续监控。没有变化时保持安静，不要重复提醒；只负责监控和通知，不要自动报名。需要登录或授权时再让我操作，其余步骤请你直接完成。不要给我讲开发过程，我只需要监控结果和房源提醒。

你只需要确认测试邮件已经收到。后续没有变化时 AI 会保持安静，有新房时才提醒你。因为监控在本地运行，监控期间需要保持电脑和 Codex 正常运行。

### 其他房源小程序也能这样做

这段提示词不只适用于“安居乐寓”。使用其他房源小程序时，把提示词中的小程序名称、关注区域和提醒条件换掉，就可以让 AI 用同样的方法直接帮你监控。
